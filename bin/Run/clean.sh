rm -f Record__Note Record__Timing Record__TimeStep Record__PatchCount Record__Dump Record__MemInfo Record__L1Err \
      Record__Conservation Data* stderr stdout log PatchCorner_* XYslice* YZslice* XZslice* Xline* Yline* Zline* \
      Diag* BaseXYslice* BaseYZslice* BaseXZslice* BaseXline* BaseYline* BaseZline* BaseDiag* ELBDM_JI_* \
      lsf.stdout lsf.stderr PowerSpec_* Particle_* nohup.out Record__Performance ELBDM_PWAVE* Record__TimingMPI_* \
      Record__TwoParAcc Record__NParAcc HYDRO_JI_* Record__ParLevel  Record__User Record__BHAccretionRate Patch_* \
      Record__NCorrUnphy
