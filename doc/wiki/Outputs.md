...
Related options:
[[--hdf5 | Installation:-Generate-Makefile#--hdf5]] &nbsp;
...
Edit `Input__Par_Mesh` to specify the desired mesh attributes. An example file can be found at `example/input/Input__Par_Mesh`.
    * **Restriction:**
Only applicable when enabling [[--tracer | Installation:-Generate-Makefile#--tracer]] and setting [OPT__OUTPUT_TOTAL](#OPT__OUTPUT_TOTAL) to 1.
...
[[--cosmic_ray | Installation:-Generate-Makefile#--cosmic_ray]]) on grids.
...
Output the SRHD Lorentz factor on grids.
    * **Restriction:**
For [[--srhd | Installation:-Generate-Makefile#--srhd]] only.
