...
#  if ( MODEL == SR_HYDRO )
   ...
   real Prim1[NCOMP_FLUID]; // store 4-velocity
   real Prim2[NCOMP_FLUID]; // store 3-velocity
#  endif
...
//  convert conserved quantities to primitive quantities
		 ...
		 Dens[PID][i][j][k] = Prim2[0];
		 Ux[PID][i][j][k]   = Prim2[1];
		 Uy[PID][i][j][k]   = Prim2[2];
		 Uz[PID][i][j][k]   = Prim2[3];
		 Engy[PID][i][j][k] = Prim2[4];
               }
