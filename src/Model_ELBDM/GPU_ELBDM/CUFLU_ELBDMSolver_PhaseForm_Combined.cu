...
uint get1D1(uint k, uint j, uint i, int XYZ) {
   switch ( XYZ )
   {
...
uint get1D2(uint k, uint j, uint i, int XYZ) {
   switch ( XYZ )
   {
...
#  if ( HYBRID_SCHEME == HYBRID_MUSCL || HYBRID_SCHEME == HYBRID_UPWIND)
   const uint ghostZonePerStage = 2; 
#  else 
   const uint ghostZonePerStage = 4; 
#  endif 
...
//       create arrays for columns of various intermediate fields on the stack
         real s_In_1PG   [FluBlockSizeY][N_TIME_LEVELS + 1][FLU_NIN][FLU_NXT];
         real s_Sr_1PG   [FluBlockSizeY][FLU_NXT]; 
         real s_Fm_1PG   [FluBlockSizeY][FLU_NXT]; 
#        ifdef CONSERVE_MASS
...
#                 ifdef GAMER_DEBUG
                  if ( s_In[ty][time_level][DENS][i] != s_In[ty][time_level][DENS][i] || s_In[ty][time_level][PHAS][i] != s_In[ty][time_level][PHAS][i] || s_In[ty][time_level][DENS][i] < 0)
