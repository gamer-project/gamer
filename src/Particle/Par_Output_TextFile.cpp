...
//
// Note        :  1. The potential attribute (ParPot) is skipped entirely, both in the header and the
//                   data rows, when OPT__OUTPUT_PAR_POT is off -- it is still kept up to date
...
#              if ( defined GRAVITY  &&  defined STORE_PAR_POT )
               if ( v == PAR_POT  &&  !OPT__OUTPUT_PAR_POT )   continue;
#              endif
               fprintf( File, BlankPlusFormat_Flt, amr->Par->AttributeFlt[v][p] );
            }
