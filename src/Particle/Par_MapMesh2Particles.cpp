...
//       calculate the nearest grid index
         for (int d=0; d<3; d++)
         {
            idx[d] = int( ( InterpParPos[d][ParID] - EdgeL[d] )*_dh );

//          prevent from round-off errors (especially for NGP and TSC)
            if ( idx[d] < 0 )
            {
#              ifdef DEBUG_PARTICLE
               if (  ! Mis_CompareRealValue( InterpParPos[d][ParID], (real)EdgeL[d], NULL, false )  )
                  Aux_Error( ERROR_INFO, "index outside the attr array (pos[%d] %14.7e, EdgeL %14.7e, idx %d) !!\n",
                             d, InterpParPos[d][ParID], EdgeL[d], idx[d] );
#              endif
            ...
            {
#              ifdef DEBUG_PARTICLE
               if (  ! Mis_CompareRealValue( InterpParPos[d][ParID], (real)EdgeR[d], NULL, false )  )
                  Aux_Error( ERROR_INFO, "index outside the attr array (pos[%d] %14.7e, EdgeR %14.7e, idx %d) !!\n",
                             d, InterpParPos[d][ParID], EdgeR[d], idx[d] );
#              endif
...
//       calculate new particle attribute
         ParAttr[ParID] = Attr3D[ idx[2] ][ idx[1] ][ idx[0] ];
...
//          calculate the array index of the left and right cells
            dr      [d] = ( InterpParPos[d[ParID] - EdgeL[d] )*_dh + ParGhost - 0.5;
            idxLR[0][d] = int( dr[d] );
...
#              ifdef DEBUG_PARTICLE
               if (  ! Mis_CompareRealValue( InterpParPos[d][ParID], (real)EdgeL[d], NULL, false )  )
                  Aux_Error( ERROR_INFO, "index outside the attr array (pos[%d] %14.7e, EdgeL %14.7e, idxL %d, idxR %d) !!\n",
                             d, InterpParPos[d][ParID], EdgeL[d], idxLR[0][d], idxLR[1][d] );
#              endif
...
#              ifdef DEBUG_PARTICLE
               if (  ! Mis_CompareRealValue( InterpParPos[d][ParID], (real)EdgeR[d], NULL, false )  )
                  Aux_Error( ERROR_INFO, "index outside the attr array (pos[%d] %14.7e, EdgeR %14.7e, idxL %d, idxR %d) !!\n",
                             d, InterpParPos[d][ParID], EdgeR[d], idxLR[0][d], idxLR[1][d] );
#              endif
...
//       calculate attribute
         ParAttr[ParID] = (real)0.0;

         for (int k=0; k<2; k++)
         for (int j=0; j<2; j++)
         for (int i=0; i<2; i++) {
            ParAttr[ParID] += Attr3D[ idxLR[k][2] ][ idxLR[j][1] ][ idxLR[i][0] ]
...
//          calculate the array index of the left, central, and right cells
            dr       [d] = ( InterpParPos[d][ParID] - EdgeL[d] )*_dh + ParGhost;
            idxLCR[1][d] = int( dr[d] );
...
#              ifdef DEBUG_PARTICLE
               if (  ! Mis_CompareRealValue( InterpParPos[d][ParID], (real)EdgeL[d], NULL, false )  )
                  Aux_Error( ERROR_INFO, "index outside the attr array (pos[%d] %14.7e, EdgeL %14.7e, idxL %d, idxR %d) !!\n",
                             d, InterpParPos[d][ParID], EdgeL[d], idxLCR[0][d], idxLCR[2][d] );
#              endif
...
#              ifdef DEBUG_PARTICLE
               if (  ! Mis_CompareRealValue( InterpParPos[d][ParID], (real)EdgeR[d], NULL, false )  )
                  Aux_Error( ERROR_INFO, "index outside the attr array (pos[%d] %14.7e, EdgeR %14.7e, idxL %d, idxR %d) !!\n",
                             d, InterpParPos[d][ParID], EdgeR[d], idxLCR[0][d], idxLCR[2][d] );
#              endif
...
//       calculate attribute
         ...
         for (int i=0; i<3; i++) {
            ...
            ParAttr[ParID] += Attr3D[ idxLCR[k][2] ][ idxLCR[j][1] ][ idxLCR[i][0] ]
               *Frac[i][0]*Frac[j][1]*Frac[k][2];
