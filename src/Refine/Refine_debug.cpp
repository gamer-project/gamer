...
#        if ( ELBDM_SCHEME == HYBRID )
            ...
            printf("an excellent interpolation!");
...
//          interpolate density
            Interpolate( &Flu_CData[DENS][0][0][0], CSize_Flu3, CStart_Flu, CRange_CC, &Flu_FData[DENS][0][0][0],
                         FSize_CC3, FStart_CC, 1, INT_CQUAR, PhaseUnwrapping_No, &Monotonicity_No,
                         IntOppSign0thOrder_No, ALL_CONS_NO, INT_PRIM_NO, INT_FIX_MONO_COEFF, NULL, NULL );



            char FileName[100];
            sprintf( FileName, "Coarse_Fluid_r%d_lv%d_p%d", MPI_Rank, lv, PID );
...
//          header
            ...
            fprintf( File, "Rank %d  Lv %d  PID %d  Time %13.7e  Step %ld  Counter %ld\n",
                     MPI_Rank, lv, PID, Time[lv], Step, AdvanceCounter[lv] );
...
#           endif 
            ...
            for (int k=0; k<CSize_Flu3[2]; k++)
            for (int j=0; j<CSize_Flu3[1]; j++)
            for (int i=0; i<CSize_Flu3[0]; i++)
            {
//             cell indices
               fprintf( File, "(%3d,%3d,%3d )", i, j, k );
               fprintf( File, "  %14.7e", Flu_CData[DENS][k][j][i] );
               fprintf( File, "  %14.7e", Flu_CData[PHAS][k][j][i] );
               fprintf( File, "\n" );
            }
            ...
            sprintf( FileName, "Fine_Fluid_r%d_lv%d_p%d", MPI_Rank, lv, PID );
...
#           endif 
            ...
            for (int k=0; k<FSize_CC3[2]; k++)
            for (int j=0; j<FSize_CC3[1]; j++)
            for (int i=0; i<FSize_CC3[0]; i++)
            {
//             cell indices
               fprintf( File, "(%3d,%3d,%3d )", i, j, k );
               fprintf( File, "  %14.7e", Flu_FData[DENS][k][j][i] );
               fprintf( File, "  %14.7e", Flu_FData[PHAS][k][j][i] );
               fprintf( File, "\n" );
               //Flu_FData[PHAS][k][j][i] -=  0.314;
            }
