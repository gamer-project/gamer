...
#     ifdef CHECK_UNPHYSICAL_IN_FLUID
      Hydro_CheckUnphysical( UNPHY_MODE_SING, &Output_1Cell[DENS], "density", ERROR_INFO, UNPHY_VERBOSE );
#     endif
...
#  endif // # ifdef GAMER_DEBUG
   ...
   for (int d=0; d<3; d++)
   {
      ...
      {
         case 0 : idx_ele_e [0] = NEleM1;  idx_ele_e [1] = NEle;
                  sizeB_i = FLU_NXT_P1; sizeB_j = FLU_NXT; sizeB_k = FLU_NXT;
                  break;
                              ...
                              const int NCon, const int NGhost, const int NEle )
{
   ...
   for (int d=0; d<3; d++)
   {
      ...
      fc[faceL][MAG_OFFSET+TDir1] += (real)0.5 * ( B_source[fL][TDir1] + B_source[fR][TDir1] );
