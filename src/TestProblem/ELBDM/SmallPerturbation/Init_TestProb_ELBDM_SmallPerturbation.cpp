...
// ********************************************************************************************************************************
   ReadPara->Add( "Perturbation_N",            &Perturbation_N,                 -1,             1,           4                    );
...
// (1-2) set the default values
   ...
   Perturbation_Wavelength = 2.0 * M_PI / amr->BoxSize[0]; // Assuming cubic box 


// (1-3) check the runtime parameters
   if ( Perturbation_N == -1 )
      Aux_Error( ERROR_INFO, "Runtime parameter \"Perturbation_N\" is not set !!\n" );
                ...
                const int lv, double AuxArray[] )
{
   ...
   for (int l = 0; l < Perturbation_N; ++l) {
      kx = Perturbation_Wavelength * (n + 1);
      ky = Perturbation_Wavelength * (m + 1);
      kz = Perturbation_Wavelength * (l + 1);
      d1 = cos(kx * x);
      d2 = cos(ky * y);
      d3 = cos(kz * z);
      d4 = sin(kx * x);
...
} // FUNCTION : Init_TestProb_ELBDM_SmallPerturbation
