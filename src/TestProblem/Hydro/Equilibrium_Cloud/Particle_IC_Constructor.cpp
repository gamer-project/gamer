...
void Particle_IC_Constructor::Load_Physical_Params(const FP filename_para,const int index){
  params.Cloud_Center   = new double[3];     // central coordinates
  params.Cloud_BulkVel  = new double[3];   // bulk velocity
    
  Aux_Message( stdout, "Reading physical parameters input file:%s\n",filename_para.Params_Filenames[index].c_str() );

  //for(int k=0;k<filenames.Cloud_Num;k++){
  // (1) load the problem-specific runtime parameters
    params.Cloud_Num                = filename_para.Cloud_Num;
    params.Params_Filenames         = filename_para.Params_Filenames[index];

    const char* FileName=filename_para.Params_Filenames[index].c_str();
    ...
    ReadPara->Add( "Cloud_Par_Num_Ratio", &params.Cloud_Par_Num_Ratio,0.,            0.,                1.0         );
    ...
    if(params.Cloud_Par_Num_Ratio==0.){
...
void Particle_IC_Constructor::Par_SetEquilibriumIC(real *Mass_AllRank, real *Pos_AllRank[3], real *Vel_AllRank[3],const long NPar_AllRank,const int Par_Idx_Prev,const int Par_Idx){
  ...
  ParM = TotM / (NPar_AllRank*params.Cloud_Par_Num_Ratio);
  ...
  for (long p=Par_Idx_Prev; p<Par_Idx; p++)
  {
     // mass
     Mass_AllRank[p] = ParM;
...
} // FUNCTION : Particle_IC_Constructor::Aux_CountRow
