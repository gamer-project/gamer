
#include "CUPOT.h"
#ifdef __CUDACC__
#include "CUDA_CheckError.h"
#endif

#ifdef GRAVITY




// =================================
// I. Set auxiliary arrays
// =================================

#ifndef __CUDACC__



//-------------------------------------------------------------------------------------------------------
// Function    :  SetExtPotAuxArray_DynamicalFriction
// Description :  Set the auxiliary arrays ExtPot_AuxArray_Flt/Int[] used by ExtPot_DynamicalFriction()
//
// Note        :  1. Invoked by Init_ExtPot_DynamicalFriction()
//                2. AuxArray_Flt/Int[] have the size of EXT_POT_NAUX_MAX defined in Macro.h (default = 20)
//                3. Add "#ifndef __CUDACC__" since this routine is only useful on CPU
//
// Parameter   :  AuxArray_Flt/Int : Floating-point/Integer arrays to be filled up
//                Time             : Target physical time
//
// Return      :  AuxArray_Flt/Int[]
//-------------------------------------------------------------------------------------------------------
void SetExtPotAuxArray_DynamicalFriction( double AuxArray_Flt[], int AuxArray_Int[], const double Time )
{
   double GC_Rho0 = 1.0;
   double GC_R0   = 0.1;
   double GC_Center[3] ={1.5,1.5,1.5};
   double GC_ExtPotMFrac = 1.0;
// potential = -G*M/(r^2+R0^2)^(1/2)
   const double ExtPotM = (4.0/3.0)*M_PI*CUBE(GC_R0)*GC_Rho0*GC_ExtPotMFrac;

   AuxArray_Flt[0] = GC_Center[0];   // x coordinate of the potential center
   AuxArray_Flt[1] = GC_Center[1];   // y ...
   AuxArray_Flt[2] = GC_Center[2];   // z ...
   AuxArray_Flt[3] = SQR( GC_R0 );   // scale_radius^2
   AuxArray_Flt[4] = -NEWTON_G*ExtPotM;   // -G*M

} // FUNCTION : SetExtPotAuxArray_DynamicalFriction
#endif // #ifndef __CUDACC__



// =================================
// II. Specify external potential
// =================================

//-----------------------------------------------------------------------------------------
// Function    :  ExtPot_DynamicalFriction
// Description :  Calculate the external potential at the given coordinates and time
//
// Note        :  1. This function is shared by CPU and GPU
//                2. Auxiliary arrays UserArray_Flt/Int[] are set by SetExtPotAuxArray_DynamicalFriction()
//                3. GenePtr has the size of EXT_POT_NGENE_MAX defined in Macro.h (default = 6)
//
// Parameter   :  x/y/z             : Target spatial coordinates
//                Time              : Target physical time
//                UserArray_Flt/Int : User-provided floating-point/integer auxiliary arrays
//                Usage             : Different usages of external potential when computing total potential on level Lv
//                                    --> EXT_POT_USAGE_ADD     : add external potential on Lv
//                                        EXT_POT_USAGE_SUB     : subtract external potential for preparing self-gravity potential on Lv-1
//                                        EXT_POT_USAGE_SUB_TINT: like SUB but for temporal interpolation
//                                    --> This parameter is useless in most cases
//                PotTable          : 3D potential table used by EXT_POT_TABLE
//                GenePtr           : Array of pointers for general potential tables
//
// Return      :  External potential at (x,y,z,Time)
//-----------------------------------------------------------------------------------------
GPU_DEVICE_NOINLINE
static real ExtPot_DynamicalFriction( const double x, const double y, const double z, const double Time,
                                     const double UserArray_Flt[], const int UserArray_Int[],
                                     const ExtPotUsage_t Usage, const real PotTable[], void **GenePtr )
{

// potential = -G*M/(r^2+R0^2)^(1/2)
   const double cx  =       UserArray_Flt[0];   // x coordinate of the potential center
   const double cy  =       UserArray_Flt[1];   // y ...
   const double cz  =       UserArray_Flt[2];   // z ...
   const real   a2  = (real)UserArray_Flt[3];   // scale_radius^2
   const real   mGM = (real)UserArray_Flt[4];   // -G*M

   const real   dx  = (real)(x - cx);
   const real   dy  = (real)(y - cy);
   const real   dz  = (real)(z - cz);
   const real   r2  = SQR(dx) + SQR(dy) + SQR(dz);
   const real   pot = mGM / SQRT( r2 + a2 );

   return pot;

} // FUNCTION : ExtPot_DynamicalFriction



// =================================
// III. Set initialization functions
// =================================

#ifdef __CUDACC__
#  define FUNC_SPACE __device__ static
#else
#  define FUNC_SPACE            static
#endif

FUNC_SPACE ExtPot_t ExtPot_Ptr = ExtPot_DynamicalFriction;

//-----------------------------------------------------------------------------------------
// Function    :  SetCPU/GPUExtPot_DynamicalFriction
// Description :  Return the function pointers of the CPU/GPU external potential routines
//
// Note        :  1. Invoked by Init_ExtPot_DynamicalFriction()
//
// Parameter   :  CPU/GPUExtPot_Ptr (call-by-reference)
//
// Return      :  CPU/GPUExtPot_Ptr
//-----------------------------------------------------------------------------------------
#ifdef __CUDACC__
__host__
void SetGPUExtPot_DynamicalFriction( ExtPot_t &GPUExtPot_Ptr )
{
   CUDA_CHECK_ERROR(  cudaMemcpyFromSymbol( &GPUExtPot_Ptr, ExtPot_Ptr, sizeof(ExtPot_t) )  );
}

#else // #ifdef __CUDACC__

void SetCPUExtPot_DynamicalFriction( ExtPot_t &CPUExtPot_Ptr )
{
   CPUExtPot_Ptr = ExtPot_Ptr;
}

#endif // #ifdef __CUDACC__ ... else ...



#ifndef __CUDACC__

// local function prototypes
void SetExtPotAuxArray_DynamicalFriction( double [], int [], const double );
void SetCPUExtPot_DynamicalFriction( ExtPot_t & );
#ifdef GPU
void SetGPUExtPot_DynamicalFriction( ExtPot_t & );
#endif

//-----------------------------------------------------------------------------------------
// Function    :  Init_ExtPot_DynamicalFriction
// Description :  Initialize external potential
//
// Note        :  1. Set auxiliary arrays by invoking SetExtPotAuxArray_*()
//                   --> They will be copied to GPU automatically in CUAPI_SetConstMemory()
//                2. Set the CPU/GPU external potential major routines by invoking SetCPU/GPUExtPot_*()
//                3. Invoked by Init_ExtAccPot()
//                   --> Enable it by linking to the function pointer "Init_ExtPot_Ptr"
//                4. Add "#ifndef __CUDACC__" since this routine is only useful on CPU
//
// Parameter   :  None
//
// Return      :  None
//-----------------------------------------------------------------------------------------
void Init_ExtPot_DynamicalFriction()
{

   SetExtPotAuxArray_DynamicalFriction( ExtPot_AuxArray_Flt, ExtPot_AuxArray_Int, Time[0] );
   SetCPUExtPot_DynamicalFriction( CPUExtPot_Ptr );
#  ifdef GPU
   SetGPUExtPot_DynamicalFriction( GPUExtPot_Ptr );
#  endif

} // FUNCTION : Init_ExtPot_DynamicalFriction

#endif // #ifndef __CUDACC__



#endif // #ifdef GRAVITY
