...
#  ifdef PARTICLE
   if ( amr->Par->FlagInit != PFLAG_MANUAL )
      Aux_Error( ERROR_INFO, "amr->Par->FlagInit (i.e., PAR_FLAG_INIT) = %d != PFLAG_MANUAL = %d !!\n",
                 amr->Par->FlagInit, PFLAG_MANUAL );
#  else
...
// warnings
   if ( MPI_Rank == 0 )
   {
      ...
      if ( OPT__BC_FLU[f] != BC_FLU_PERIODIC )
         Aux_Message( stderr, "WARNING : periodic BC for fluid is recommended for this test !!\n" );
   } // if ( MPI_Rank == 0 )
...
// overwrite the total number of particles
   amr->Par->NPar_Active_AllRank = ( OPT__FLAG_PAR_TARGET == FLAG_PAR_BOTH ) ? 5 : 3;
   PRINT_RESET_PARA( amr->Par->NPar_Active_AllRank, FORMAT_LONG, "(PAR_NPAR in Input__Parameter)" );

// refine by must-refine particles if present; otherwise, refine by gas density
   OPT__FLAG_RHO = ( OPT__FLAG_PAR_TARGET == FLAG_PAR_CAN  ||  OPT__FLAG_PAR_TARGET == FLAG_PAR_NONE ) ? true : false;

// reload the density flag table in case OPT__FLAG_RHO changes
   if ( OPT__FLAG_RHO )
   {
...
// only density is relevant for this test due to OPT__FLAG_RHO
   Dens = ParFlag_DensL + ( ParFlag_DensR - ParFlag_DensL )*( x - amr->BoxEdgeL[0] )/amr->BoxSize[0];
   MomX = 0.0;
   MomY = 0.0;
   MomZ = 0.0;
   Pres = 1.0;
...
//-------------------------------------------------------------------------------------------------------
// Function    :  Init_TestProb_Hydro_ParFlag
