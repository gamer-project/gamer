...
static double
    SinkParTest_theta_B; // The angle between the magnetic field and the z-axis
...
void Validate() {
  ...
  if (MPI_Rank == 0) {
    ...
    Aux_Message(
        stderr,
        "WARNING : it's recommended to enable DUAL_ENERGY for this test\n");
#endif

    for (int f = 0; f < 6; f++)
      if (amr->BoxSize[0] != amr->BoxSize[1] ||
          amr->BoxSize[0] != amr->BoxSize[2])
...
// Return      :  None
//-------------------------------------------------------------------------------------------------------
void LoadInputTestProb(const LoadParaMode_t load_mode, ReadPara_t *ReadPara,
                       HDF5_Output_t *HDF5_InputTest) {

#ifndef SUPPORT_HDF5
  if (load_mode == LOAD_HDF5_OUTPUT)
    Aux_Error(ERROR_INFO, "please turn on SUPPORT_HDF5 in the Makefile for "
                          "load_mode == LOAD_HDF5_OUTPUT !!\n");
#endif

  if (load_mode == LOAD_READPARA && ReadPara == NULL)
    Aux_Error(ERROR_INFO,
              "load_mode == LOAD_READPARA and ReadPara == NULL !!\n");
  if (load_mode == LOAD_HDF5_OUTPUT && HDF5_InputTest == NULL)
    Aux_Error(ERROR_INFO,
              "load_mode == LOAD_HDF5_OUTPUT and HDF5_InputTest == NULL !!\n");
...
void Load_Turbulence_SinkParTest() {
  ...
  tur_table_NBin = Aux_LoadTable(tur_table, Tur_Table, 6, TargetCols,
                                 RowMajor_No, AllocMem_Yes);
  ...
  Vrms =
      SQRT((Total_VelX_SQR + Total_VelY_SQR + Total_VelZ_SQR) / tur_table_NBin -
           SQR((Total_VelX + Total_VelY + Total_VelZ) / tur_table_NBin));
  Vrms_Scale = SinkParTest_Mach_num * SinkParTest_Cs / Vrms;

  // Rescale velocity
  for (int i = 0; i < tur_table_NBin; i++) {
    Table_Rescaled_VelX[i] =
        Vrms_Scale * (Table_VelX[i] - Total_VelX / tur_table_NBin);
    Table_Rescaled_VelY[i] =
        Vrms_Scale * (Table_VelY[i] - Total_VelY / tur_table_NBin);
    Table_Rescaled_VelZ[i] =
        Vrms_Scale * (Table_VelZ[i] - Total_VelZ / tur_table_NBin);
  }
...
void SetParameter() {
  ...
  if ((size - 1 < NX0_TOT[0]) || (size - 1 < NX0_TOT[1]) ||
      (size - 1 < NX0_TOT[2]))
    Aux_Error(ERROR_INFO,
              "size - 1 should be smaller than half of NX0_TOT_X/Y/Z !!\n");

  // (2) set the problem-specific derived parameters
  SinkParTest_Core_Mass *= Const_Msun / UNIT_M;
  SinkParTest_Cs = SQRT((Const_kB * ISO_TEMP / UNIT_E) /
                        (MOLECULAR_WEIGHT * Const_amu / UNIT_M));
  SinkParTest_R0 /= UNIT_L;
  SinkParTest_Rho0 =
      3.0 * SinkParTest_Core_Mass / (4.0 * M_PI * CUBE(SinkParTest_R0));
...
void SetGridIC(real fluid[], const double x, const double y, const double z,
  ...
  if (Rs < SinkParTest_R0) {
    Dens = SinkParTest_Rho0 *
           (1 + SinkParTest_Delta_Dens * COS(2 * ATAN(dy / dx)));
  ...
  MomY = Dens * VelY;
  MomZ = Dens * VelZ;
  Etot = Hydro_ConEint2Etot(Dens, MomX, MomY, MomZ, Eint,
                            0.0); // do NOT include magnetic energy here
...
// Return      :  magnetic
//-------------------------------------------------------------------------------------------------------
void SetBFieldIC(real magnetic[], const double x, const double y,
                 const double z, const double Time, const int lv,
                 double AuxArray[]) {
...
#ifdef PARTICLE
void Par_Init_ByFunction_SinkParTest(
    const long NPar_ThisRank, const long NPar_AllRank, real_par *ParMass,
...
void Init_TestProb_Hydro_SinkParTest() {
  ...
  Par_Init_ByFunction_Ptr =
      Par_Init_ByFunction_SinkParTest; // option: PAR_INIT=1; example:
                                       // Particle/Par_Init_ByFunction.cpp
#endif
#if (EOS == EOS_USER)
  EoS_Init_Ptr =
      EoS_Init_Barotropic_SinkParTest; // option: EOS in the Makefile; example:
                                       // EoS/User_Template/CPU_EoS_User_Template.cpp
  EoS_End_Ptr = NULL;
#endif
#ifdef SUPPORT_HDF5
