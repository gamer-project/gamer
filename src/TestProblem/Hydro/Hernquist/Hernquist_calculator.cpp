...
double potential_Hernquist(double x){
  return - (Hernquist_NEWTON_G*Hernquist_Rho0*Hernquist_R0*Hernquist_R0)  / (x+1);
}
double rho_dx_Hernquist(double x){
  return - Hernquist_Rho0 *(1/(x*x*(1+x)*(1+x)*(1+x)) + 3/(x*(1+x)*(1+x)*(1+x)*(1+x))) /(2*M_PI);
}
double de_rho_over_de_psi_Hernquist(double x) {
  double rho_dx_Hernquist =  Hernquist_Rho0 *(1/(x*x*(1+x)*(1+x)*(1+x)) + 3/(x*(1+x)*(1+x)*(1+x)*(1+x))) /(2*M_PI);
  double psi_dx_Hernquist = Hernquist_NEWTON_G*Hernquist_R0*Hernquist_R0*Hernquist_Rho0*pow( 1+x,-2 );
      
  return rho_dx_Hernquist/psi_dx_Hernquist;
}

//GSL functions
double psi_potential_Hernquist(double r, void * parameters){
  double psi= *(double *)parameters;
  return psi+potential_Hernquist(r);
}
...
//Probability Density
double integration_Hernquist(double eng){
  //double epson=0.1;
  double min = 0;
  double max = eng;
  int num=1000;
...
double integration_eng_base_Hernquist(double eng){
  //double x0 =inverse_psi_to_x_Hernquist(eng);
  return integration_Hernquist(eng);
}
void Hernquist_calculator::initialize_prob_dens(){
  double min =-potential_Hernquist(100),max =-potential_Hernquist(0.001);/***difference***/
...
//Principal functions
void Hernquist_calculator::init(double newton_g,double rho,double r){
