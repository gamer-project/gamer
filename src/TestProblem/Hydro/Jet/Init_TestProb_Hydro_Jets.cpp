...
// errors
#  if ( MODEL != HYDRO )
   Aux_Error( ERROR_INFO, "MODEL must be HYDRO !!\n" );
#  endif
...
// (1-2) convert to code unit
   Jet_SrcVel               *= Const_c / sqrt( 1.0 + Jet_SrcVel*Jet_SrcVel ) / UNIT_V;
   Jet_SrcTemp              *= Const_GeV / UNIT_E;
   ...
   else if ( Jet_Ambient == 0 )
   {
     ...
     Jet_UniformVel[0]      *= Const_c   / sqrt( 1.0 + SQR(Jet_UniformVel[0]) ) / UNIT_V;
     Jet_UniformVel[1]      *= Const_c   / sqrt( 1.0 + SQR(Jet_UniformVel[1]) ) / UNIT_V;
     Jet_UniformVel[2]      *= Const_c   / sqrt( 1.0 + SQR(Jet_UniformVel[2]) ) / UNIT_V;
   }
   else if ( Jet_Ambient == 3 )
   {
   ...
   Jet_UniformTemp          *= Const_GeV / UNIT_E;
   ...
   if ( Jet_TimeDependentSrc )
   {
     ...
     Jet_Burst4Vel          *= Const_c / sqrt( 1.0 + SQR(Jet_Burst4Vel) ) / UNIT_V;
     Jet_BurstDens          *= 1.0          / UNIT_D;
     Jet_BurstTemp          *= Const_GeV    / UNIT_E;
   }
...
// (4) make a note
   ...
   if ( Jet_Ambient == 0 && MPI_Rank == 0 )
   {
     ...
     Aux_Message( stdout, "  Jet_UniformTemp         = %14.7e GeV\n",        Jet_UniformTemp*UNIT_E/Const_GeV );
     Aux_Message( stdout, "  Jet_UniformVel[x]       = %14.7e cm/s\n",       Jet_UniformVel[0]*UNIT_V                        );
     Aux_Message( stdout, "  Jet_UniformVel[y]       = %14.7e cm/s\n",       Jet_UniformVel[1]*UNIT_V                        );
     Aux_Message( stdout, "  Jet_UniformVel[z]       = %14.7e cm/s\n",       Jet_UniformVel[2]*UNIT_V                        );
...
// uniform ambient
   if ( Jet_Ambient == 0 )
   {
	  ...
	  Pri4Vel[4] = Jet_UniformTemp * Jet_UniformDens/(Jet_ParticleMassSrc/UNIT_M);
   ...
   if ( Jet_Ambient == 1 )
   {
      ...
      if ( Jet_HSE_Radius > 0.0 )
      {
         if ( r <= Jet_HSE_Radius )
         {
           ...
           Pri4Vel[4] = Temp * Pri4Vel[0]/(Jet_ParticleMassSrc/UNIT_M);
         }
         else
         {
           ...
           Pri4Vel[4] = Pri4Vel[0] / DropRatio * Temp/(Jet_ParticleMassSrc/UNIT_M);
         }
      }
      else
      {
...
#     ifdef GRAVITY
      ...
      if ( Jet_HSE_Radius > 0.0 )
      {
        if ( r <= Jet_HSE_Radius )
        {
	      ...
	      Pri4Vel[0] /= SQR( r );
	      Pri4Vel[4]  = Jet_UniformTemp * Pri4Vel[0]/(Jet_ParticleMassSrc/UNIT_M);
        }
        else
        {
	      Pri4Vel[0]  = Jet_UniformTemp / (2.0*M_PI*NEWTON_G);
          ...
          Pri4Vel[0] *= DropRatio;
          Pri4Vel[4]  = Pri4Vel[0] * Jet_UniformTemp / DropRatio/(Jet_ParticleMassSrc/UNIT_M);
        }
      }
      else
      {
...
#     endif
   ...
   Pri4Vel_real[0] = (real)Pri4Vel[0];
   Pri4Vel_real[1] = (real)Pri4Vel[1];
   Pri4Vel_real[2] = (real)Pri4Vel[2];
   Pri4Vel_real[3] = (real)Pri4Vel[3];
   Pri4Vel_real[4] = (real)Pri4Vel[4];


   Hydro_Pri2Con( Pri4Vel_real, fluid, (real)(1.0/(GAMMA - 1.0)), false, 0, NULL );
                                         ...
                                         const int lv, double AuxArray[] )
{
	...
	|| ( InsideLowerCone && ( Jet_Exist == 2 || Jet_Exist == 3 ) ) )
  {
    if ( Jet_HalfOpeningAngle == 0.0 )
      ...
      Prim[4] = Jet_SrcTemp*Jet_SrcDens/(Jet_ParticleMassSrc/UNIT_M);
  	}

    real Pri4Vel_real[5];

    Pri4Vel_real[0] = (real)Prim[0];
    Pri4Vel_real[1] = (real)Prim[1];
    Pri4Vel_real[2] = (real)Prim[2];
    Pri4Vel_real[3] = (real)Prim[3];
    Pri4Vel_real[4] = (real)Prim[4];
