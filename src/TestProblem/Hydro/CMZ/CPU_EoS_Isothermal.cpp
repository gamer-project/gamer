...
void EoS_SetAuxArray_Isothermal( double AuxArray[] )
{
   ...
   const double T = 1.0e4; // should be a runtime parameter in the future

// kB*T/m = Cs^2
   AuxArray[0] = ( Const_kB*T/UNIT_E ) / ( MOLECULAR_WEIGHT*Const_amu/UNIT_M );

   if ( MPI_Rank == 0 )
   Aux_Message( stdout, "Cs = %21.14e km/s\n", SQRT(AuxArray[0])*UNIT_V/Const_km );
...
#  endif // GAMER_DEBUG
   ...
   const real kT_m = AuxArray[0];
   const real Pres = kT_m*Dens;
...
#  endif // GAMER_DEBUG
   ...
   const real Eint = (real)1.667*Pres;   // assuming gamma=5/3 but doesn't really matter since Eint is useless anyway
