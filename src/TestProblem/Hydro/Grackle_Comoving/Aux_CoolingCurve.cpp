...
// create a cooling table at the specified redshift on the proper coordinate
...
void Compute_CoolingCurve( const double z_value, const double T_min, const double T_max, const double dT )
{
  ...
  my_fields.DI_density   [0] = grackle_data->HydrogenFractionByMass * grackle_data->DeuteriumToHydrogenRatio * my_fields.density[0];
  my_fields.DII_density  [0] = 0.0;
  ...
  *********************************************************************/
  ...
  Aux_Message( stdout, "  done %s (maxiter = %d)\n", ofname, maxiter );
