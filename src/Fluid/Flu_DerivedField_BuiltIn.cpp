...
#ifdef VISCOSITY
//-------------------------------------------------------------------------------------------------------
// Function    :  Flu_DerivedField_Mu
// Description :  Define the built-in dynamic viscosity derived field
//
// Note        :  1. Array size:
//                      Out  [] : NFieldOut*(NCellInX-2*NGhost)*(NCellInY-2*NGhost)*(NCellInZ-2*NGhost)
//                      FluIn[] : NCOMP_TOTAL*NCellInX*NCellInY*NCellInZ
...
#  endif // #ifdef GAMER_DEBUG
   ...
   real u[NCOMP_TOTAL], Emag, Temp, mu, nu;
...
#     endif
      ...
      Hydro_ComputeViscosity( mu, nu, &MicroPhy, u[DENS], Temp );
...
// fill in the output array
   ...
   real B[NCOMP_MAG], vp, vm, B2, DeltaP;

   const real _2dh = (real)0.5 / (real)dh;

   Flu_DerivedField_Mu( Out, FluIn, MagIn, NFieldOut, NCellInX, NCellInY, NCellInZ,
                        NGhost, dh );
   ...
   for (int io=0; io<NCellOutX; io++)  {  const int ii = io + NGhost;
      ...
      for (int v=0; v<NCOMP_MAG; v++)  B[v] = MagIn3D[v][ki][ji][ii];
      B2 = (  SQR( B[MAGX] ) + SQR( B[MAGY] ) + SQR( B[MAGZ] );
      ...
      DeltaP = Out3D[0][ko][jo][io]*(3.0*BBdV - divV);
      if ( VISCOSITY_BOUNDS )
