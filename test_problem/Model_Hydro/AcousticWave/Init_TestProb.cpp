...
static void HYDRO_TestProbSol_AcousticWave( real fluid[], const double x, const double y, const double z,
                                                         const double Time );
static void HYDRO_OutputError_AcousticWave( const bool BaseOnly );
...
// global variables in the HYDRO acoustic wave test
// =======================================================================================
...
// Description :  Initialize parameters for the HYDRO acoustic wave test
//
...
//                   "HYDRO_TestProbSol_AcousticWave"
//
// Parameter   :  None
//-------------------------------------------------------------------------------------------------------
void Init_TestProb()
{
   ...
   const char *TestProb = "HYDRO acoustic wave";

// check
#  if ( MODEL != HYDRO )
#  error : ERROR : "MODEL != HYDRO" in the HYDRO acoustic wave test !!
#  endif

#  ifdef GRAVITY
#  error : ERROR : "GRAVITY must be OFF" in the HYDRO acoustic wave test !!
#  endif

#  ifndef FLOAT8
#  error : ERROR : "FLOAT8 must be ON" in the HYDRO acoustic wave test !!
#  endif

#  ifdef COMOVING
#  error : ERROR : "COMOVING must be OFF" in the HYDRO acoustic wave test !!
#  endif

#  ifdef PARTICLE
#  error : ERROR : "PARTICLE must be OFF" in the HYDRO acoustic wave test !!
#  endif
...
// set the initialization and output functions
   Init_Function_Ptr      = HYDRO_TestProbSol_AcousticWave;
   Output_TestProbErr_Ptr = HYDRO_OutputError_AcousticWave;
...
// record the test problem parameters
   if ( MPI_Rank == 0 )
   {
      ...
      Aux_Message( stdout, "=============================================================================\n" );
      Aux_Message( stdout, "NOTE : density amplitude   = % 14.7e\n", Acoustic_RhoAmp );
...
//-------------------------------------------------------------------------------------------------------
// Function    :  HYDRO_TestProbSol_AcousticWave
// Description :  Calculate the analytical solution in the HYDRO acoustic wave test
//
...
// Return      :  fluid
//-------------------------------------------------------------------------------------------------------
void HYDRO_TestProbSol_AcousticWave( real fluid[], const double x, const double y, const double z, const double Time )
{
...
//-------------------------------------------------------------------------------------------------------
// Function    :  HYDRO_OutputError_AcousticWave
// Description :  Compare and output the numerical and analytical solutions in the HYDRO acoustic wave test problem
//
...
//-------------------------------------------------------------------------------------------------------
void HYDRO_OutputError_AcousticWave( const bool BaseOnly )
{
...
// output file name
   ...
   sprintf( FileName[0], "HYDRO_ACOUSTIC_DENS_%d%d%d%d%d%d", ID[0], ID[1], ID[2], ID[3], ID[4], ID[5] );
   sprintf( FileName[1], "HYDRO_ACOUSTIC_MOMX_%d%d%d%d%d%d", ID[0], ID[1], ID[2], ID[3], ID[4], ID[5] );
   sprintf( FileName[2], "HYDRO_ACOUSTIC_MOMY_%d%d%d%d%d%d", ID[0], ID[1], ID[2], ID[3], ID[4], ID[5] );
   sprintf( FileName[3], "HYDRO_ACOUSTIC_MOMZ_%d%d%d%d%d%d", ID[0], ID[1], ID[2], ID[3], ID[4], ID[5] );
   sprintf( FileName[4], "HYDRO_ACOUSTIC_PRES_%d%d%d%d%d%d", ID[0], ID[1], ID[2], ID[3], ID[4], ID[5] );
// ===================================================================================================
...
// ===================================================================================================
   HYDRO_TestProbSol_AcousticWave( Anal, x, y, z, Time[0] );
// ===================================================================================================
...
void LoadTestProbParameter()
{
   ...
   getline( &input_line, &len, File );
   sscanf( input_line, "%lf%s",  &Acoustic_RhoAmp,    string );

   getline( &input_line, &len, File );
   sscanf( input_line, "%lf%s",  &Acoustic_Cs,        string );

   getline( &input_line, &len, File );
   sscanf( input_line, "%lf%s",  &Acoustic_v0,        string );

   getline( &input_line, &len, File );
   sscanf( input_line, "%lf%s",  &Acoustic_Sign,      string );

   getline( &input_line, &len, File );
   sscanf( input_line, "%lf%s",  &Acoustic_Phase0,    string );
