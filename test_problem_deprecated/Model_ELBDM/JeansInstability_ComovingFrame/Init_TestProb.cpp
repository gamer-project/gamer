...
static void ELBDM_TestProbSol_JeansInstability_Comoving( real fluid[], const double x, const double y, const double z,
                                                         const double Time );
static void ELBDM_OutputError_JeansInstability_Comoving( const bool BaseOnly );
...
// global variables in the ELBDM comoving-frame Jeans instability test
// =======================================================================================
static real Jeans_WaveLength;    // wave length
static real Jeans_WaveK;         // wave number
static real Jeans_WaveKj;        // critical wave number
static real Jeans_RealAmp0;      // proportional coefficient of the wave real part amplitude
static real Jeans_ImagAmp0;      // proportional coefficient of the wave imaginary part amplitude
...
// =======================================================================================
inline real Jeans_RealAmp( const real RealAmp0, const real y )
{
   const real y2 = y*y;
...
inline real Jeans_ImagAmp( const real ImagAmp0, const real y )
{
   const real y2 = y*y;
   const real y3 = y*y2;
   const real y4 = y2*y2;
...
// Description :  Initialize parameters for the ELBDM comoving-frame Jeans instability test
//
// Note        :  Please copy this file to "GAMER/src/Init/Init_TestProb.cpp"
//
// Parameter   :  None
//-------------------------------------------------------------------------------------------------------
void Init_TestProb()
{
   ...
   const char *TestProb = "ELBDM comoving-frame Jeans instability";

// check
#  if ( MODEL != ELBDM )
#  error : ERROR : "MODEL != ELBDM" in the ELBDM comoving-frame Jeans instability test !!
#  endif

#  ifndef GRAVITY
#  error : ERROR : "GRAVITY must be ON" in the ELBDM comoving-frame Jeans instability test !!
#  endif

#  ifndef COMOVING
#  error : ERROR : "COMOVING must be ON" in the ELBDM comoving-frame Jeans instability test !!
#  endif
...
// set the initialization and output functions
   Init_Function_Ptr      = ELBDM_TestProbSol_JeansInstability_Comoving;
   Output_TestProbErr_Ptr = ELBDM_OutputError_JeansInstability_Comoving;
...
// set global variables
   ...
   Jeans_Phase0     = 0.0;
   Jeans_RealAmp0   = 4.0e-10;   // for UNSTABLE solution
// Jeans_RealAmp0   = 4.0e-08;   // for   STABLE solution
...
// record the test problem parameters
   if ( MPI_Rank == 0 )
   {
      const real y = Jeans_WaveK*Jeans_WaveK/ELBDM_ETA*pow( Time[0], -0.5 );
...
// End_T : (stable/unstable) --> (1 period in the high-k limit/grow by a factor of 50 in the low-k limit)
   const double End_T_Default    = ( Jeans_Stable) ?
                                    A_INIT + pow( double(0.5*Jeans_WaveK*Jeans_WaveK/M_PI/ELBDM_ETA), 2.0 ) :
                                    A_INIT*50;
...
//-------------------------------------------------------------------------------------------------------
// Function    :  ELBDM_TestProbSol_JeansInstability_Comoving
// Description :  Calculate the analytical solution in the ELBDM comoving-frame Jeans instability test
//
// Note        :  1. Wave vector is along the diagonal direction
//                2. Background density is assumed to be ONE
//                3. This function is invoked by "ELBDM_Init_StartOver_AssignData" and
//                   "ELBDM_OutputError_JeansInstability_Comoving"
//
...
// Return      :  fluid
//
// Reference   :  Woo, T., & Chiueh, T. 2009, ApJ, 697, 850
//-------------------------------------------------------------------------------------------------------
void ELBDM_TestProbSol_JeansInstability_Comoving( real fluid [], const double x, const double y, const double z, const double Time )
{

   const double r       = 1.0/sqrt(3.0)*( x + y + z );
   const double Jeans_y = Jeans_WaveK*Jeans_WaveK/ELBDM_ETA*pow( Time, -0.5 );
...
} // FUNCTION : ELBDM_TestProbSol_JeansInstability



//-------------------------------------------------------------------------------------------------------
// Function    :  ELBDM_OutputError_JeansInstability_Comoving
// Description :  Compare and output the numerical and analytical solutions in the ELBDM comoving-frame
...
//-------------------------------------------------------------------------------------------------------
void ELBDM_OutputError_JeansInstability_Comoving( const bool BaseOnly )
{
...
// ===================================================================================================
   ELBDM_TestProbSol_JeansInstability_Comoving( Anal, x, y, z, Time[0] );
// ===================================================================================================
