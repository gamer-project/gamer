...
from X_some_function import read_selected_parameters,read_parameters
import datetime
...
if DATA_SOURE == 'SIMU':
    ...
    params_TB = read_parameters("../Input__TestProb")
    m = params_TB['GC_MASS']     # mass of the massive object in [Msun]
    GC_velocity = params_TB['GC_VELX'], params_TB['GC_VELY'], params_TB['GC_VELZ'] # [pc/Myr] = [kpc/Gyr]
    GC_position = params_TB['GC_POSX']/1000, params_TB['GC_POSY']/1000, params_TB['GC_POSZ']/1000 # [kpc]
    Halo_Center = params_TB['Cloud_CenterX']/1000, params_TB['Cloud_CenterY']/1000, params_TB['Cloud_CenterZ']/1000 # [kpc]
    ...
    Interpolate_table_path = params_TB['Chandrasekhar_DF_Table_Name']
    halo_rs,rho0_halo, halo_type = params_TB['HALO_Rs']/1000, params_TB['HALO_RHO_0']*1E9,params_TB['HALO_TYPE']# [kpc], [Msun/kpc^3], Type
    stellar_rs, rho0_stellar, stellar_type = params_TB['STELLAR_Rs']/1000, params_TB['STELLAR_RHO_0']*1E9,params_TB['STELLAR_TYPE']# [kpc], [Msun/kpc^3], Type
...
version = 'interpolate_table' # 'analytical' or 'interpolate_table' -> 'analytical' means using the analyical result "without" any hard truncation, 'interpolate_table' means using the table generated by A_SET_IC.py

LN_LAMBDA_BMAX_FACTOR = 3 # BMAX fitting factor for ln_Lambda, 3 is the default value meaning b_max = r/3
...
def accumulated_mass_from_file(r):
    ...
    # print(index, Input_Radius[index], Input_Mass_Profile[index])
...
def density_from_file(r):
    ...
    # print(index, Input_Radius[index], density)
...
if WRITE_TO_FILE:
    filename = f'Chandrasekhar_result/bminFactor={LN_LAMBDA_BMIN_FACTOR}_bmaxFactor={LN_LAMBDA_BMAX_FACTOR}_{date}.txt'
    with open(filename, 'w') as f:
        f.write(f'# t_end={t_end}, dt={dt}\n')
        f.write(f'# r0={r0}, v0={v0}\n')
        f.write(f'# t r\n')
        for i in range(len(r_history)):
            f.write(f'{t_history[i]}\t{r_history[i]}\n')
...
if SAVE_PLOT_RESULT:
    ...
    plt.savefig(f'Chandrasekhar_result/bminFactor={LN_LAMBDA_BMIN_FACTOR}_bmaxFactor={LN_LAMBDA_BMAX_FACTOR}_{date}.png', dpi=300)
    # plt.show()
    print('Save the plot result...Done! : '+f'Chandrasekhar_result/bminFactor={LN_LAMBDA_BMIN_FACTOR}_bmaxFactor={LN_LAMBDA_BMAX_FACTOR}_{date}.png')
print('========================================================')
