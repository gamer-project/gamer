...
parser = argparse.ArgumentParser( description='Make multi-AMR-level UM_IC uniform \n\
                                  Example usage: python3 Make_UM_IC_uniform.py -input UM_IC_input -output UM_IC_output -n0_x_in 256 -l0_x_in 1.0 -level_out 0')
...
parser.add_argument( '-n0_x_in', action='store', required=True, type=int, dest='n0_x_in',
                     help='number of input UM_IC base level grids' )
...
UM_IC_Input_N_x_base                = args.n0_x_in                # number of base-level cells in the input UM_IC in the x direction
UM_IC_Input_N_y_base                = args.n0_x_in                # ...                                                  y direction
                                                                  # (or set manually if the box is not cubic
UM_IC_Input_N_z_base                = args.n0_x_in                # ...                                                  z direction
                                                                  # (or set manually if the box is not cubic
...
def Repeated_Data(UM_IC_Input_LowerLevel, lv):
    ...
    if delta_lv < 0:
        print("ERROR: lv must <= Target_lv for repeating!")
        return np.nan
...
def Averaged_Data(UM_IC_Input_HigherLevel, lv):
    ...
    if delta_lv < 0:
        print("ERROR: lv must >= Target_lv for averaging!")
        return np.nan
...
for lv in range(0, UM_IC_Input_NLEVEL, 1):
    ...
    if lv < Target_lv:    # Lower levels
        ...
        if Method_Lv_LtoH == 1:    # Repeat
            ...
            print("ERROR: Unsported Method_Lv_LtoH!")
        ...
        if Method_Lv_Same == 1:    # Paste
            ...
            print("ERROR: Unsported Method_Lv_Same!")

    elif lv > Target_lv:  # Higher levels
        ...
        if Method_Lv_HtoL == 1:    # Pass
            ...
            print("ERROR: Unsported Method_Lv_HtoL!")

    else:
        print("ERROR: Unknown lv")
