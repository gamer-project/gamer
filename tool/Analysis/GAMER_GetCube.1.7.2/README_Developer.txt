...
GAMER_GetCube : construct the uniform-mesh data from a GAMER output file by spatial interpolation
...
3. New "MultiFiles__GAMER_GetCube.sh" script
...
1. output x=[100,500], y=[1000,1234], z=5, level=2 plane (with MinMod limiter interpolation) using 8 processes
   mpirun -np 8 ./GAMER_GetCube -i InputFile -o Comment -l 2 -n 3 -x 100 -X 400 -y 1000 -Y 234 -z 5 -p 2 -q 2 -r 2

2. output x projection, y=[0,BoxSize], z=[0,BoxSize], level=5 (with central interpolation), binary data 
   using 4 processes
   mpirun -np 4 ./GAMER_GetCube -i InputFile -o Comment -l 5 -n 4 -I 1 -r 4 -b

3. output cube, x=[x0,x0+dx], y=[y0,y0+dy], z=[z0,z0+dz], level=3, binary data using 1 process
   mpirun -np 1 ./GAMER_GetCube -i InputFile -o Comment -l 3 -n 7 -b -x x0 -X dx -y y0 -Y dy -z z0 -Z dz
