...
GAMER_GetCube : construct the uniform-mesh data from the GAMER output file
...
1. This tool can be used to cut a slice or 3D rectangle from the GAMER output
   ...
   file of GAMER_GetCube will contain a uniform-mesh data with the spatial
...
3. Usage demo:
   ...
   (1) Output the xy plane with x=[100:500], y=[1000:1234], and z=5 at level 2.
       ...
       mpirun -np 8 ./GAMER_GetCube -i Input -l 2 -n 3 -x 100 -X 400 
   ...
   (2) Output the x projection with y=[0:BoxSize] and z=[0:BoxSize] at level 5.
       ...
       mpirun -np 4 ./GAMER_GetCube -i Input -l 5 -n 4 -I 1 -r 4 -b
   ...
   (3) Output the 3D data with x=[x0:x0+dx], y=[y0:y0+dy], and z=[z0:z0+dz] 
       ...
       mpirun -np 1 ./GAMER_GetCube -i Input -o .txt -l 3 -n 7  
